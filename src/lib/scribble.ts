...
// A TAP IS A FINGER THAT DID NOT MOVE (#361).
//
// Roman: "sometimes a stroke I draw disappears immediately after I make it. Is it
// because I draw too fast?" It was, and this is why.
//
// When the pen comes up the app decides: tap, or line? A tap becomes a dot. It
// used to decide with two tests — under 400ms, and never more than THIRTY pixels
// from where it started. Not the length of the line drawn: how far it ever got
// from its beginning. So a quick little tick, a small circle, a short dash back
// towards itself, was called a tap. The mark was thrown away and a dot put at the
// start point. On screen: a stroke vanishing the instant it is made, and only
// when you are quick, because a slower mark passed the time test.
//
// Speed says nothing about what a person meant. Movement does. A tap is now a
// finger that essentially did not move — a few pixels of wobble, the same for a
// finger as for a pencil — and how long it rested there does not matter.
// Anything that travelled is a mark, and is kept as one, beginning and end.
...
/**
 * SEND WHEN THE HAND STOPS, NOT ONCE PER STROKE (#381).
 *
 * Every stroke used to go up the moment it was finished, so twenty quick marks
...
function sendWhenTheHandStops(): void {
  if (_sendTimer !== null) clearTimeout(_sendTimer);
  _sendTimer = window.setTimeout(() => {
    _sendTimer = null;
    void flushSyncNow();
  }, SEND_AFTER_THE_HAND_STOPS_MS);
}
...
/**
 * THE PEN SAYS WHEN IT IS DOWN (#362, part of).
 *
 * Only the saying, for now. Nothing acts on it here: holding the page redraw back
 * while a stroke is in progress was tried and taken out again, because the layer
 * ended up not being rebuilt at all and the test said so. The fault is real and
 * caught — see the test marked as still open — but the fix is not, so it is not
 * shipped.
 *
 * The flag itself is worth setting: the version-drawing canvas has honoured it
 * for a while, so a picture finishing its decode no longer repaints over a
 * scribble stroke in progress.
 */
...
function eraseWithPath(eraserPath: { x: number; y: number }[], cvs: HTMLCanvasElement): void {
  ...
  if (eraseEntries.length > 0) {
    ...
    bumpRenderTick();
    sendWhenTheHandStops();   // rubbing out is drawing too (#381)
  }
...
function wirePageDrawing(cvs: HTMLCanvasElement): void {
  ...
  function onUp(e: PointerEvent): void {
    ...
    // Movement alone decides (#361) — see the note by TAP_MAX_DISTANCE.
    const wasTap = dist < TAP_MAX_DISTANCE;
    if (wasTap) {
