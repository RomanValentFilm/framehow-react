...
export function renderVersionFrame(div: HTMLElement, fid: number): void {
  ...
  div.innerHTML = `
    <div class="frame-num ver-frame-num">${
      f && f.label ? `<span class="frame-label-tag ver-label-combo" data-editverlabel="${fid}">${f.label} ${f.stripLabels?.ver || 'version'}</span>` : '<span></span>'
    }<div class="version-tabs${
    s.reorderFid === fid ? ' locked-dim' : s.verReorderFid === fid ? ' locked' : ''
  }">${tabsHTML}</div>${
    ...
    f?.cropW || 960
      ...
      <div class="version-actions"${_verHidden ? ' style="pointer-events:none;opacity:0.3;"' : ''}>
        ...
        <button class="act-btn${_verHidden ? ' disabled' : s.prevFrameState[fid] && s.prevFrameState[fid]!.origin === 'ver' ? '' : ' disabled'}" ${_verHidden ? '' : 'data-action="undo" data-fid="' + fid + '"'}><svg width="12" height="12" viewBox="-24 -24 48 48" fill="currentColor"><g transform="scale(-1,1) rotate(90)"><path d="M-12,-18 L-12,2 A14,14 0 0 0 16,2 L16,-6 L10,-6 L10,2 A8,8 0 0 1 -6,2 L-6,-12 L-1,-12 L-9,-22 L-17,-12 L-12,-12 Z"/></g></svg></button>
      </div>
    </div>`;
  ...
  div.querySelectorAll('[data-editverlabel]').forEach((el) =>
    el.addEventListener('click', async () => {
      if (!f) return;
      const result = await showVerLabelEdit(f.label, f.stripLabels?.ver || 'version');
      if (result === null) return;
      if (!f.stripLabels) f.stripLabels = {};
      f.stripLabels.ver = result;
      renderVersionFrame(div, fid);
    })
  );
