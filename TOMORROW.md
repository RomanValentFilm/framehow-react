...
### NEXT SESSION — EMAILS AND ACCOUNTS (brief, 22 Sept 15:45)
...
- DELETING AN ACCOUNT, Roman's rules: press → signed out everywhere, cannot sign in, space stops counting · 7 days → everything still there · after that the nightly sweep erases projects, pictures, restore points, the account row AND its analytics rows (purgeDeletedAccounts in cleanup.ts, called from the cron after the expired projects). The address stays reserved during the 7 days (signup now looks at deleted accounts too).
- Analytics: "Deleted accounts (N)" link under All Users → its own page with days left and a Restore button (restore-account: clears deleted_at on the user and on the projects deleted in the same moment).
- Analytics: "New password" button per user row → word-pair password shown ONCE (raven-velvet-93), old password dead, all sessions dropped, reset links dropped. This is the beta's answer to "I forgot my password" while no mail can be sent — Roman knows every tester personally.
- Forgot-password box rewritten: no longer promises a letter; gives info@framehow.com and a "Write to us" button that opens the person's own mail program ready-written.
- The question box (confirm) now sits above every overlay (was 10001, beaten by strip tag / needs / notes / label boxes). Roman's sighting was NOT reproduced — test 34 proves the question is the element in front on desktop.
- Terms tick box on Create account: unticked, refused until ticked, links to /terms and /privacy (placeholder pages built by postbuild from legal-terms.html and legal-privacy.html — ROMAN'S OWN TEXT STILL TO COME). The moment of agreement goes to the server and is kept in users.terms_agreed_at (migration 0029, applied live by hand).
- Test 34 `e2e/34-account.spec.ts` (~12 s) covers all of it.
- NOTE: `npx wrangler login` was needed again for the D1 command (the deploy path works, the database path did not).

### 22 September — v4.9.237 · #540 DEPLOYED ~15:35, app only — a save with no settings memory dates nothing as new; next number v4.9.238 · #541
- `seedSettingsAgeUnknown(projectId)` in projectSettings.ts: every item changed_at=UNKNOWN, serverAt=UNKNOWN. Used at boot (loadSnapshot) and in putCopyInPlace when the save/copy has no settingStamps AND the project has a cloud id. A project never on the server keeps seedSettings(lastModified). Log: "settings memory: none in the save — N item(s) recorded as AGE UNKNOWN".
- Door `forgetSettingsMemoryInSave()` (testHooks + harness) strips settingStamps from the local save. Test 32 round 6: desktop renames a third time; ipad strips memory, reloads → takes the desktop's names, pushes nothing back; server holds ENSEMBLE/PETRA.
- Roman's Testman press on 236 WORKED: ~408 MB freed. Check Storage tomorrow (~385 MB bucket) and that the 3 a.m. sweep took DLSparks' 7 files.
