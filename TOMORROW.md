...
### 18 September — v4.9.219 · #522 (app AND backend). Full run 293: 75 green · 6 skipped · 1 red (15, the known idle-timing one: the tablet was not touched and did not ask).
...
7. Restore modal: SAVE RESTORE POINT sits at the TOP of the list, always, and saves NOW (flushes unsent work first); the server makes no new "where you are now" point when nothing changed since the newest one (`unchanged: true`), so reopening the modal no longer breeds "now" rows.
