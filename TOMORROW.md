...
dev is **v4.9.237 · #540** app (deployed 22 Sept ~15:35); backend is **v4.9.236 · #539** (last backend change). Next number: **v4.9.238 · #541**. Deploys 22 Sept: 232 (app), 233–236 (app+backend), 237 (app). Runs 22 Sept: 316–321 test 32, 322 numbering + big day 9/10, 323–327 test 33, 329 test 32 six rounds — all green at the end. NEXT: (a) the FULL RUN on 237 — start before bed, Mac awake: `FH_RUN=330 npm run t` (~70 min); (b) EMAILS (provider + secrets + finish send(); verify + forgot-password by hand); (c) LATER list; (d) before launch: sync log off on framehow.com.
